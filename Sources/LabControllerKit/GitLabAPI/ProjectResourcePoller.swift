...
/// 第一頁的最大值取整後就等於現行游標，游標不會動，下一輪讀到同一段區間。
/// （**未必是同一批資源**——時間戳完全相同者順序未定，每輪可能回不同的子集合；下面說的正是這件事。）
///
...
/// ## 已知缺口
///
/// - **後面某一頁出錯會作廢整輪**：解碼失敗與分頁標頭不一致（例如快取回了別頁）都以拋錯表達，
///   於是第一頁明明已經讀好、游標本來可以前進，
...
public struct ProjectResourcePoller: Sendable {
    ...
    /// （見呼叫端）。**撞到預算不算**：那多半只是上限**比資料早**了不到一秒（`Date` 標頭是整秒精度，
    /// 站台在 09:00:10.8 發出的就是 `09:00:10`，而資料可能是 09:00:11.2），下一輪就推得動；
    /// 而「後面還有頁」本來就該用 `truncated` 表達，那個 case 也不宣稱游標動了。
