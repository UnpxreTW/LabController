...
/// 幾行，落地成本可忽略。
///
/// - Warning: 精確的保證範圍：`synchronize()`（`fsync`）把資料交到**核心**手上，因此行程當掉、
///   核心 panic 都不會讓已回傳的 append 消失；但在 Darwin 上 `fsync` **不保證碟片已實際寫入**，
///   突然斷電仍可能掉最後幾筆——要那一級得用 `F_FULLFSYNC`，代價高出一個量級。
///
...
public actor DecisionLogFile {
    ...
    /// 新檔的權限：僅擁有者可讀寫。
    ///
    /// - Important: 這**不是**秘密的邊界——依 schema 設計，紀錄裡沒有任何欄位裝得下秘密，遮蔽
    ///   留在上游的 trace 層。這裡防的是另一件事：日誌記著專案路徑、事件鑰匙、處置理由這類營運
    ///   內情，同機其他帳號沒有理由讀得到。屬縱深防禦、不是唯一防線。
    ...
    ///
    /// - Note: 若真有另一個寫入者在這次判斷與寫入之間補了東西，最壞情況是多寫一個換行、在日誌裡
    ///   留下一個空行——讀取端本來就會跳過空行，不會變成壞資料。
