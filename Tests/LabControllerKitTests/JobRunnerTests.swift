...
private final class JobRunnerTests {
	...
	/// 全部步驟綠燈時判完成，收尾把環境焚毀。
	@Test
	private func `completes a job whose steps all succeed`() async throws {
	...
	@Test
	private func `places the workspace in the guest at spawn`() async {
		let backend: InMemoryExecutionBackend = .init()
		let plan: JobPlan = .init(
			jobIdentifier: 1,
			steps: [.init(name: "script", script: ["swift build"])],
	...
	@Test
	private func `fails the job when a required step exits non-zero`() async {
	...
	@Test
	private func `skips on-success steps after a failure but still runs the always ones`() async {
		let backend: InMemoryExecutionBackend = .init(script: .init(handler: { command in
	...
	/// 允許失敗的步驟紅了不讓整個 job 紅。
	@Test
	private func `keeps the job green when a step that may fail exits non-zero`() async {
		let backend: InMemoryExecutionBackend = .init(script: .init(handler: { command in
	...
	@Test
	private func `reports a system failure when the checkout does not succeed`() async {
		let backend: InMemoryExecutionBackend = .init(script: .init(handler: { command in
	...
	@Test
	private func `reports a system failure when the backend cannot run the command`() async {
	...
	@Test
	private func `stops before opening a guest when the workspace cannot be rendered`() async throws {
		let backend: InMemoryExecutionBackend = .init()
		let plan: JobPlan = .init(
			jobIdentifier: 1,
	...
	@Test
	private func `keeps the script failure as the cause when the budget runs out afterwards`() async {
		let backend: InMemoryExecutionBackend = .init(script: .init(handler: { command in
	...
	@Test
	private func `keeps the finished result when the guest cannot be destroyed`() async {
	...
	@Test
	private func `masks secret values that a command prints`() async {
	...
	@Test
	private func `writes skipped capabilities into the trace`() async {
		let backend: InMemoryExecutionBackend = .init()
		let plan: JobPlan = .init(
			jobIdentifier: 1,
			steps: [.init(name: "script", script: ["swift build"])],
