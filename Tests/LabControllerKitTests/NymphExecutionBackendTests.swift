...
// MARK: - NymphExecutionBackendTests

private final class NymphExecutionBackendTests {

	/// 開一台 guest 送出去的那一行，逐欄就是對面認得的形狀（含必填的 os 欄與不帶結尾換行）。
	@Test
	private func `sends the spawn request in the shape the daemon accepts`() async throws {
		...
		let expected: [String: Any] = try object(.init(#"""
			...
			"""#.utf8))
		#expect(try same(parameters(of: line, verb: "spawn"), expected))
	}

	/// 組態裡的 guest 種類與規模原樣送出，不被本側改寫。
	@Test
	private func `carries the configured guest kind and size`() async throws {
		let transport: ScriptedNymphTransport = .init([readySpawn])
		...
		let sent: [String: Any] = try parameters(of: firstLine(of: transport), verb: "spawn")
	...
	@Test
	private func `rejects a host path image without asking the daemon`() async throws {
		let transport: ScriptedNymphTransport = .init([readySpawn])
	...
	@Test
	private func `destroys the guest it could not bring up`() async throws {
	...
	@Test
	private func `writes injected files after the guest is up`() async throws {
		...
		let written: [String: Any] = try parameters(
			of: #require(transport.requests.withLock { $0.last }), verb: "execute"
		)
	...
	@Test
	private func `destroys the guest when an injected file cannot be written`() async throws {
		let refused: String = #"""
	...
	/// 沒有注入檔就不多送任何一行。
	@Test
	private func `sends nothing extra when there is no file to inject`() async throws {
		let transport: ScriptedNymphTransport = .init([readySpawn])
		let backend: NymphExecutionBackend = .init(transport: transport, configuration: configuration)
	...
	@Test
	private func `returns a non zero exit code as data`() async throws {
		let failed: String = #"""
		...
		let sent: [String: Any] = try parameters(of: firstLine(of: transport), verb: "execute")
	...
	@Test
	private func `maps the unknown identifier error onto the protocol`() async throws {
	...
	/// 還沒就緒的那則翻成「還在、但收不了命令」。
	@Test
	private func `maps the not ready error onto a guest that cannot take commands`() async throws {
	...
	@Test
	private func `treats an unrecognised error code as an unusable backend`() async throws {
	...
	@Test
	private func `reports an unreachable daemon as an unusable backend`() async throws {
	...
	@Test
	private func `fails loudly when the response does not match the verb`() async throws {
	...
	@Test
	private func `lists every guest including the stopped ones`() async throws {
		let sessions: String = #"""
			{"list":{"_0":{"sessions":[
		...
		#expect(try parameters(of: firstLine(of: transport), verb: "list")["all"] as? Bool == true)
	...
	/// 查單一 guest 走同一套翻譯。
	@Test
	private func `reads a single guest through the same translation`() async throws {
		let single: String = #"""
	...
	/// 查不到的那台在 status 上是錯誤。
	@Test
	private func `reports an unknown guest when asked for its status`() async throws {
	...
	/// 清除硬停、不等它自己收尾。
	@Test
	private func `destroys a guest without waiting for it to wind down`() async throws {
		let transport: ScriptedNymphTransport = .init([destroyed])
		let backend: NymphExecutionBackend = .init(transport: transport, configuration: configuration)
		try await backend.destroy(.init("mfly-1"))
		let sent: [String: Any] = try parameters(of: firstLine(of: transport), verb: "destroy")
	...
	@Test
	private func `treats destroying a guest that is already gone as done`() async throws {
	...
	@Test
	private func `still throws when destroying fails for another reason`() async throws {
	...
	@Test
	private func `cleans up the guest when the work is done`() async throws {
