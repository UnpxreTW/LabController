...
private final class RunCommandTests {
	...
	@Test
	private func `run parses the required options`() throws {
	...
	@Test
	private func `run carries the resource options into the backend configuration`() throws {
		let parsed: any ParsableCommand = try LabControllerCommand.parseAsRoot(minimalArguments + [
	...
	@Test
	private func `run resolves the socket path`() throws {
	...
	@Test
	private func `run has no option that takes the token itself`() {
		#expect(throws: (any Error).self) {
	...
	@Test
	private func `existing commands still parse`() throws {
